...
<section id="main" class="column">
    ...
    <article class="module width_full">
        <header><h3 class="tabs_involved">Список уроков</h3>
        </header>
        ...
        <table class="tablesorter" cellspacing="0">
            ...
            <tbody>
            <c:forEach items="${scheduleTimes}" var="st">
                <tr>
                    <td><c:out value="${st.lessonNum}"/></td>
                    <td><c:out value="${st.sdt}"/></td>
                    <td><c:out value="${st.durationMin}"/></td>
                    <td><input type="image" src="<c:url value="/images/icn_edit.png"/>" data-toggle="modal" data-target="#modalTimeEdit_${st.id}" title="Редактировать"><a href="<c:url value="/times/delete?id=${st.id}"/>"><img src="<c:url value="/images/icn_trash.png"/>" title="Удалить"></a></td>
                </tr>


                <!-- Modal -->
                <div class="modal fade" id="modalTimeEdit_${st.id}" role="dialog">
                    <div class="modal-dialog">
                        ...
                        <div class="modal-content">
                            ...
                            <form name="edit_group" action="/times/edit?id=${st.id}" method="POST">
                                <div class="modal-body">
                                    <div class="row">
                                        <fieldset style="width:48%; float:left; margin-right: 3%">
                                            <div class="col-md-11">
                                                <div class="form-group">
                                                    <label for="inputLessonNum">Номер урока</label>
                                                    <input class="form-control input-sm" id="inputLessonNum" type="number" maxlength="99" minlength="0" name="j_lessonNum"
                                                           value="${st.lessonNum}"/>
                                                </div>
                                            </div>
                                        </fieldset>
                                        <fieldset style="width:48%; float:left;">
                                            <div class="col-md-11">
                                                <div class="form-group">
                                                    <label for="inputSdt">Время начала</label>
                                                    <input class="form-control input-sm" id="inputSdt" type="time" name="j_sdt"
                                                           value="${st.sdt}"/>
                                                </div>
                                            </div>
                                        </fieldset>
                                        ...
                                        <fieldset style="width:48%; float:left; margin-right: 3%">
                                            <div class="col-md-11">
                                                <div class="form-group">
                                                    ...
                                                    <input class="form-control input-sm" id="inputDuration" type="number" minlength="0" name="j_duration"
        ...
        <ul class="pagination">
            <c:if test="${curPage == 0}">
                <li class="disabled"><a href="/admin/time/list?page=0">&laquo;</a></li>
            </c:if>
            <c:if test="${curPage > 0}">
                <li><a href="/admin/time/list?page=0">&laquo;</a></li>
            </c:if>
            ...
            <c:forEach var="i" begin="${begin}" end="${totalPages}">
                <c:if test="${i <= curPage+3}">
                    <c:if test="${curPage == i}">
                        <li class="active"><a href="/admin/time/list?page=${i}"><c:out value="${i+1}"/>
                            <span class="sr-only">(current)</span></a></li>
                    </c:if>
                    <c:if test="${curPage != i}">
                        <li><a href="/admin/time/list?page=${i}"><c:out value="${i+1}"/> <span
            ...
            <c:if test="${curPage == totalPages}">
                <li class="disabled"><a href="/admin/time/list?page=${totalPages}">&raquo;</a>
                </li>
            </c:if>
            <c:if test="${curPage < totalPages}">
                <li><a href="/admin/time/list?page=${totalPages}">&raquo;</a></li>
            </c:if>
        </ul>
        <!--end Pages -->
