...
public class Test extends Thread {
    ...
    public void run() {
        try {
            if (weekDayName == null) {
                realTimeaddHomeWorskAndJurnals();
            } else {
    ...
    public void addTeacher(String teacherName, String teacherSurname, String teacherLogin) {
    ...
    public void addHomeWorks(int countWeeks, WeekDayName dayOfWeek) throws ParseException {
        ...
        for (int i = 0; i < countWeeks; i++) {
            for (Schedule s :
                ...
                if (random.nextInt() % 3 == 0) {
                    StudentsGroup studentsGroup = groupService.getGroupById(s.getStudentsGroup().getId());
                    if (studentService.getStudentByGroup(studentsGroup).size() > 0) {
                        String sdt = s.getScheduleTimes().getSdt();
                        Date date = dateFormat.parse(sdt);
                        int hh = date.getHours();
                        int mm = date.getMinutes();
                        ...
                        Teacher teacher = teacherService.findById(s.getTeacher().getId());
                        Subject subject = subjectService.findById(s.getSubject().getId());

                        homeWorkService.addOrUpdateHomeWork(new HomeWork(createDate, studentsGroup, teacher, subject, generateHomeWorkDescription()));
                    }
                }
                try {
                    Thread.sleep(2000);
    ...
    public void realTimeaddHomeWorskAndJurnals() {

        WeekDayName curentWeekDay = null;
        Calendar curentTime = new GregorianCalendar();
        for (WeekDayName n :
                WeekDayName.values()) {
            if (n.getcalendarWeekDay() == curentTime.get(Calendar.DAY_OF_WEEK)) {
                curentWeekDay = n;
            }
        }
        List<Schedule> schedules = scheduleService.findByWeekDayName(curentWeekDay);

        for (Schedule s : schedules) {
            StudentsGroup studentsGroup = groupService.getGroupById(s.getStudentsGroup().getId());
            Teacher teacher = teacherService.findById(s.getTeacher().getId());
            Subject subject = s.getSubject();
            String desription = generateHomeWorkDescription();
            Random random = new Random();
            if (random.nextInt() % 3 == 0) {
                HomeWork homeWork = new HomeWork(curentTime, studentsGroup, teacher, subject, desription);
            ...
            for (Student st :
                ...
                if (random.nextInt() % 2 == 0) {
                    ...
                    Student student = studentService.getStudentById(st.getId());
                    MarkStamp markStamp = new MarkStamp(m, mt);
                    Jurnal jurnal = new Jurnal(subject, teacher, markStamp, student);
                    jurnals.add(jurnal);
                }
            }
